    ...
    <v-bottom-sheet v-model="this.$store.state.sheet" persistent>
      ...
      <v-card color="blue-grey">
        <v-layout align-center justify-center row class="grey darken-2">
          <v-card-title
            class="title white--text" primary-title>
            ...
            <v-stepper-items>
              <v-stepper-content step="1">
                    ...
                    <v-card-text>
                        <v-container fluid>
                            <v-layout row wrap align-center>
                              ...
                              <v-flex xs6>
                                <v-select v-model="select" :hint="`${select.state}, ${select.abbr}`"
                                  :items="items" item-text="state" item-value="abbr" label="Select"
                ...
                <v-layout align-center justify-center row pt-2>
                    <v-btn block color="primary" @click="continueone()" class="mr-1">
                      Continue
                    </v-btn>
              ...
              <v-stepper-content step="2">
                <v-card>
                    ...
                    <v-divider></v-divider>
                    <v-card-text>
                        <v-data-table :headers="headers" :items="desserts" :search="search" v-model="selected"
                ...
                <v-divider></v-divider>
                <v-layout align-center justify-center row pt-2>
                    <v-btn block color="primary" @click="continuetwo()" class="mr-1">
                      Continue
                    </v-btn>
              ...
              <v-stepper-content step="3">
                <v-card>
                    <v-card-title primary-title class="title">Choose your Template</v-card-title>
                    <v-divider></v-divider>
                    <v-card-text>
                ...
                <v-divider></v-divider>
                <v-layout align-center justify-center row pt-2>
                    <v-btn block color="primary" class="mr-1" disabled>
                      Continue
                    </v-btn>
...
// var testLayout = { 'x': 0, 'y': 0, 'w': 6, 'h': 8, 'i': '0' };
export default {
  data() {
    return {
      dialog: false,
      layout :[
          { 'x': 0, 'y': 0, 'w': 6, 'h': 8, 'i': '0' ,'newwidth': '','newheight': '','CardType':'Count'},
          { 'x': 6, 'y': 0, 'w': 2, 'h': 8, 'i': '1' ,'newwidth': '','newheight': '',  'CardType':'CountTable'},
          { 'x': 8, 'y': 0, 'w': 4, 'h': 8, 'i': '2' ,'newwidth': '','newheight': '','CardType':'Count'},
          { 'x': 0, 'y': 8, 'w': 7, 'h': 3, 'i': '3' ,'newwidth': '','newheight': '','CardType':'Count'},
          { 'x': 7, 'y': 8, 'w': 5, 'h': 9, 'i': '4' ,'newwidth': '','newheight': '','CardType':'CountTable'},
          { 'x': 0, 'y': 11, 'w': 3, 'h': 6, 'i': '5' ,'newwidth': '','newheight': '','CardType':'Count'},
          { 'x': 3, 'y': 11, 'w': 4, 'h': 6, 'i': '6' ,'newwidth': '','newheight': '','CardType':'CountTable'},
      ],
      draggable: true,
      resizable: true,
      comList : [],
  ...
  methods:{
    ...
    submit(cardType) {
      let newCard = { 'x': 0, 'y': 0, 'w': 6, 'h': 8, 'i': this.layout.length.toString() ,'CardType':cardType}
      // this.layout.push(newCard)
      this.layout.splice(0,0,newCard);
